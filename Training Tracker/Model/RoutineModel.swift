            ...
            if exerciseGroup.exercises == nil {
                ...
                for index in 0..<exerciseSequence.count-1 {
                    ...
                    if let restInBetween = exerciseGroup.restInBetween {
                        if restInBetween > 0 {
                            exerciseWithRestSequence.append(Step(name: "Rest in between: \(restInBetween)", stepType: StepType.timed(TimedStepDefinition(type: .rest, duration: TimeInterval(restInBetween)))))
                        }
                    }
                }
                ...
                if let restAtTheEnd = exerciseGroup.restAtTheEnd {
                    if restAtTheEnd > 0 {
                        exerciseWithRestSequence.append(Step(name: "Rest at the end: \(restAtTheEnd)", stepType: StepType.timed(TimedStepDefinition(type: .rest, duration: TimeInterval(restAtTheEnd)))))
                    }
                }
            ...
            for exerciseIndex in 0..<exerciseList.count {
                if setIndex < exerciseList[exerciseIndex].numberOfSets {
                    if let durations = exerciseList[exerciseIndex].durations {
                        let step = Step(name: exerciseList[exerciseIndex].name, description: exerciseList[exerciseIndex].description, stepType: .timed(TimedStepDefinition(type: .exercise, prepTime: toOptionalTimeInterval(exerciseList[exerciseIndex].prepTimes?[setIndex]), duration: toOptionalTimeInterval(durations[setIndex]))))
